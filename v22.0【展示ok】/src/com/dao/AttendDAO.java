 ...
 * 
 * @see org.sf.model.Attend
 * @author MyEclipse Persistence Tools
 */

public class AttendDAO extends BaseHibernateDAO {
	private static final Logger log = LoggerFactory.getLogger(AttendDAO.class);
	...
	public Attend findById(AttendId id) {
		log.debug("getting Attend instance with id: " + id);
		try {
			Attend instance = (Attend) getSession().get("org.sf.model.Attend",
					id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(Attend instance) {
		...
		try {
			List results = getSession().createCriteria("org.sf.model.Attend")
	...
	public Attend merge(Attend detachedInstance) {
		...
		try {
			Attend result = (Attend) getSession().merge(detachedInstance);
